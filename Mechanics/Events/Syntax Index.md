
| Code   | Arguments                                      | Description                                            | Details                                                   |
| ------ | ---------------------------------------------- | ------------------------------------------------------ | --------------------------------------------------------- |
| `0x0d` | 0: ???<br>1: ???<br>2: ???<br>3: ???<br>4: ??? | Camera?                                                |                                                           |
| `0x0e` | 0: Pose index<br>1:<br>2:                      | Change actor's pose                                    |                                                           |
| `0x19` | -                                              | Move north 0.5 steps                                   |                                                           |
| `0x1a` | -                                              | Move north 1 step                                      |                                                           |
| `0x1b` | -                                              | Move north 1.5 steps                                   |                                                           |
| `0x1c` | -                                              | Move north 2 steps                                     |                                                           |
| `0x1d` | -                                              | Move north 2.5 steps                                   |                                                           |
| `0x1e` | -                                              | Move north 3 steps                                     |                                                           |
| `0x1f` | -                                              | Move north 3.5 steps                                   |                                                           |
| `0x29` | -                                              | Move west 0.5 steps                                    |                                                           |
| `0x2a` | -                                              | Move west 1 step                                       |                                                           |
| `0x2b` | -                                              | Move west 1.5 steps                                    |                                                           |
| `0x2c` | -                                              | Move west 2 steps                                      |                                                           |
| `0x2d` | -                                              | Move west 2.5 steps                                    |                                                           |
| `0x2e` | -                                              | Move west 3 steps                                      |                                                           |
| `0x2f` | -                                              | Move west 3.5 steps                                    |                                                           |
| `0x39` | -                                              | Move south 0.5 steps                                   |                                                           |
| `0x3a` | -                                              | Move south 1 step                                      |                                                           |
| `0x3b` | -                                              | Move south 1.5 steps                                   |                                                           |
| `0x3c` | -                                              | Move south 2 steps                                     |                                                           |
| `0x3d` | -                                              | Move south 2.5 steps                                   |                                                           |
| `0x3e` | -                                              | Move south 3 steps                                     |                                                           |
| `0x3f` | -                                              | Move south 3.5 steps                                   |                                                           |
| `0x49` | -                                              | Move east 0.5 steps                                    |                                                           |
| `0x4a` | -                                              | Move east 1 step                                       |                                                           |
| `0x4b` | -                                              | Move east 1.5 steps                                    |                                                           |
| `0x4c` | -                                              | Move east 2 steps                                      |                                                           |
| `0x4d` | -                                              | Move east 2.5 steps                                    |                                                           |
| `0x4e` | -                                              | Move east 3 steps                                      |                                                           |
| `0x4f` | -                                              | Move east 3.5 steps                                    |                                                           |
| `0x50` | -                                              | Wait 2 frames                                          |                                                           |
| `0x51` | -                                              | Wait 6 frames                                          |                                                           |
| `0x52` | -                                              | Wait 8 frames                                          |                                                           |
| `0x53` | -                                              | Wait 10 frames                                         |                                                           |
| `0x54` | -                                              | Wait 12 frames                                         |                                                           |
| `0x55` | -                                              | Wait 14 frames                                         |                                                           |
| `0x56` | -                                              | Wait 16 frames                                         |                                                           |
| `0x57` | -                                              | Wait 18 frames                                         |                                                           |
| `0x58` | -                                              | Wait 20 frames                                         |                                                           |
| `0x59` | -                                              | Wait 22 frames                                         |                                                           |
| `0x5a` | -                                              | Wait 24 frames                                         |                                                           |
| `0x5b` | -                                              | Wait 26 frames                                         |                                                           |
| `0x5c` | -                                              | Wait 28 frames                                         |                                                           |
| `0x5d` | -                                              | Wait 30 frames                                         |                                                           |
| `0x5e` | -                                              | Wait 32 frames                                         |                                                           |
| `0x5f` | -                                              | Wait 34 frames                                         |                                                           |
| `0x91` | 0: x position<br>1: y position                 | Show an exclamation mark                               |                                                           |
| `0x92` | 0: x position<br>1: y position                 | Show a question mark                                   |                                                           |
| `0x93` | 0: x position<br>1: y position                 | Show a sweat drop                                      |                                                           |
| `0x94` | 0: x position<br>1: y position                 | Show a flashing quote sign                             |                                                           |
| `0x95` | 0: x position<br>1: y position                 | Show an impact effect                                  |                                                           |
| `0xa0` | 0: Signal ID                                   | Broadcast a signal                                     | [[Signals]]                                               |
| `0xa1` | 0: Signal ID                                   | Receive a signal                                       | [[Signals]]                                               |
| `0xa2` | -                                              | Increment broadcast counter                            | [[Signals]]                                               |
| `0xb2` | 0: Sound index                                 | Play sound effect                                      |                                                           |
| `0xde` | 0: Function ID                                 | Execute a function                                     |                                                           |
| `0xe3` | 0: String index                                | Shows the text given by the string index as a text box |                                                           |
| `0xe8` | 0: Motion speed                                | Sets the actor's motion speed                          |                                                           |
| `0xea` | 0: Red<br>1: Green<br>2: Blue                  | Modify the color channel values of the actor's texture |                                                           |
| `0xeb` | -                                              | Toggle visibility                                      |                                                           |
| `0xfa` | 0: ???<br>1: ???<br>2: ???<br>3: ???           | Walk??? Camera???                                      |                                                           |
| 0xff   |                                                | Terminator?                                            | Any `0x00` byte that appear after this command is padding |

